...
class ReportsViewModel extends ChangeNotifier {
  ...
  Future<void> _loadData() async {
    ...
    if (_campaigns.isNotEmpty) {
      _selectedCampaignId = _campaigns.first.id;
      _healthRecords = await _recordRepo.getRecordsByCampaign(
        _selectedCampaignId,
      );
    }
  ...
  Future<void> refreshReport(BuildContext context) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Đã cập nhật dữ liệu báo cáo mới nhất!'),
  ...
  Future<void> exportToPdf(
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Đang tạo file PDF...')));
    try {
  ...
  Future<void> exportToExcel(
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Đang tạo file Excel...')));
    try {
  ...
  Map<String, int> get classCompletionStats {
    ...
    for (var student in _students) {
      if (student.campaignId == _selectedCampaignId &&
          completedStudentIds.contains(student.id)) {
  ...
  // ✨ ĐÃ FIX: Chỉ đếm số liệu khi sinh viên ĐÃ KHÁM đúng trạm tương ứng


  Map<String, dynamic> get overviewStats {
    if (_isLoading || _selectedCampaignId.isEmpty) return {};

    // 1. Lọc sinh viên theo lớp và chiến dịch
    final filteredStudents = _students.where((s) {
      final matchesClass =
          _selectedClass == 'all' || s.className == _selectedClass;
      final inCampaign = s.campaignId == _selectedCampaignId;
      return matchesClass && inCampaign;
    }).toList();

    // 2. Lấy danh sách hồ sơ của các sinh viên hợp lệ
    final validRecords = _healthRecords
        .where((r) => filteredStudents.any((s) => s.id == r.studentId))
        .toList();

    int completedAll = 0;
    int poorHealth = 0; // Từ Trạm 4
    int highBp = 0; // Từ Trạm 3
    int lowBp = 0; // Từ Trạm 3
    int poorVision = 0; // Từ Trạm 2

    for (var r in validRecords) {
      // Đếm số SV hoàn thành cả 4 trạm
      if (r.completedStations.length == 4) {
        completedAll++;
      }

      // ✨ KIỂM TRA TRẠM 4 (Khám tổng quát): Thể trạng TB/Kém
      if (r.general != null) {
        final healthStatus = r.general!['healthStatus'];
        // Chỉ đếm nếu bác sĩ chọn Trung bình ('average') hoặc Kém ('poor')
        if (healthStatus == 'average' || healthStatus == 'poor') {
          poorHealth++;
        }
      }

      // ✨ KIỂM TRA TRẠM 3 (Đo huyết áp): Cao / Thấp
      if (r.bloodPressure != null) {
        final sys = r.bloodPressure!['systolic'] as num?;
        final dia = r.bloodPressure!['diastolic'] as num?;
      ...
      // ✨ KIỂM TRA TRẠM 2 (Khám thị lực): Giảm thị lực (Dưới 7/10)
      if (r.vision != null) {
        final leftStr = r.vision!['leftEye']?.toString() ?? '10/10';
        final rightStr = r.vision!['rightEye']?.toString() ?? '10/10';

        // Hàm chuyển "7/10" thành số 7 để so sánh
        double parseVision(String v) {
          try {
            return double.parse(v.split('/')[0]);
          } catch (_) {
            return 10;
          }
        }

        // Nếu 1 trong 2 mắt có thị lực <= 7/10 thì đưa vào danh sách cần chú ý
        if (parseVision(leftStr) <= 7 || parseVision(rightStr) <= 7) {
          poorVision++;
        }
      }
    }
  ...
  Map<String, dynamic>? get bmiStats {
    ...
    // 1. Lọc sinh viên theo lớp và chiến dịch được chọn
    final filteredStudents = _students.where((s) {
      final matchesClass =
          _selectedClass == 'all' || s.className == _selectedClass;
      final inCampaign = s.campaignId == _selectedCampaignId;
    ...
    // 2. ✨ ĐÃ SỬA: Lấy hồ sơ chỉ cần có khám thể lực (physical != null) là tính, không bắt ép phải đủ 4 trạm
    final validRecords = _healthRecords
        .where(
          (r) =>
              r.physical != null &&
    ...
    for (var r in validRecords) {
      ...
      if (category == 'underweight')
        ...
        overweight++;
      else if (category == 'obese_1' || category == 'obese')
        obese1++;
      else if (category == 'obese_2')
        obese2++;
    }
