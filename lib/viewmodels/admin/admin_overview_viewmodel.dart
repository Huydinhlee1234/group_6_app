...
class AdminOverviewViewModel extends ChangeNotifier {
  ...
  // ✨ ĐÃ FIX LỖI: Lọc bỏ số liệu của sinh viên đã bị xóa
  List<Map<String, dynamic>> get stationStats {
    // 1. Lấy danh sách ID của các sinh viên CÒN TỒN TẠI trong hệ thống
    final validStudentIds = _students.map((s) => s.id).toSet();

    return _stations.map((station) {
      // 2. Chỉ đếm những hồ sơ có student_id nằm trong danh sách hợp lệ
      final completed = _healthRecords.where((r) =>
      validStudentIds.contains(r.studentId) &&
          r.completedStations.contains(station.id)
      ).length;
